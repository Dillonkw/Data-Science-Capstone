...
#Uses the reduce function to full join the 2021-2023 datasets list by the SEQN variable
FullData_21_23 <- reduce(datasets_21_23, full_join, by = "SEQN") 

#Inserts a new column called Survey Year
...
CleanedData <- ComData %>% #Storing the full data into a new dataset called CleanedData
  ...
  filter(URXPREG != 1 | is.na(URXPREG)) #keeps people that are not pregnant 

#Selecting all the variable that are relevant to the research question
...
#Combines the two nutrient variables into one total variable
CleanedData <- CleanedData %>%
  mutate(
...
#View distrbution of BMI
table(CleanedData$BMI, useNA = "ifany")


#Identify total missing values for each variable
...
#Putting skewed variables into a vector 
log_columns <- c(
...
BMI_recipe <- recipe(BMI_Binary ~ ., data = train_data_clean) %>%
  ...
  step_smote(BMI_Binary)


#Creating five fold cross validation folds and stratifying it by the outcome variable
set.seed(Seed) 
...
logreg_test_results <- logreg_test_results %>% 
  mutate(.pred_class = factor(if_else(.pred_Healthy >= 0.2, "Healthy", "Unhealthy"), 
...
logreg_conf_matrix_new_thres <- conf_mat(logreg_test_results, 
                                         truth = BMI_Binary, 
                                         estimate = .pred_class)
...
rf_test_results <- rf_test_results %>% 
  mutate(.pred_class = factor(if_else(.pred_Healthy >= 0.33, "Healthy", "Unhealthy"), 
                              levels = c("Healthy", "Unhealthy")))
...
#Confusion Matrix for new threshold
rf_conf_matrix_new_thres <- conf_mat(rf_test_results, 
                                     truth = BMI_Binary, 
                                     estimate = .pred_class)
...
xg_test_results <- xg_test_results %>% 
  mutate(.pred_class = factor(if_else(.pred_Healthy >= 0.5, "Healthy", "Unhelathy"), 
                              levels = c("Healthy", "Unhealthy")))
...
#Confusion Matrix for new threshold
xg_conf_matrix_new_thres <- conf_mat(xg_test_results, 
                                     truth = BMI_Binary, 
                                     estimate = .pred_class)
